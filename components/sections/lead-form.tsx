...
export function LeadFormPanel({
  ...
  return (
    <div className={cn("mx-auto max-w-xl", className)}>
      <LeadFormCard embedded={embedded} />
    </div>
  );
}

function LeadFormCard({ embedded }: { embedded?: boolean }) {
  ...
  if (status === "success") {
    return <SuccessPanel name={fullName.trim()} embedded={embedded} />;
  }

  const shell = embedded
    ? "p-4 md:p-5"
    : "rounded-3xl border border-border bg-card p-6 shadow-md md:p-8";
    ...
    <div className={shell}>
      <form onSubmit={onSubmit} noValidate>
        ...
        <Button
          type="submit"
          size="lg"
          className={cn("mt-7 w-full", embedded && "lf-express-btn")}
...
function SuccessPanel({ name, embedded }: { name: string; embedded?: boolean }) {
  ...
  return (
    <div
      className={cn(
        ...
        embedded
          ? "p-4"
          : "rounded-[24px] border border-border bg-card p-6 shadow-md md:p-9",
      )}
      role="status"
      aria-live="polite"
    >
